			...
			else if( events[i].events & EPOLLIN )
			{
				...
				while( true )
				{
					...
					else
					{
						...
						if( ( users[sockfd].buf[idx-2] != '\r' ) || ( users[sockfd].buf[idx-1] != '\n' ) )
						{
							continue;
						}
						...
						{
							printf("execl work\n");
							//1.exel之前让标准输出指向sockfd
							//这样execl 执行的的结果对标准输出的内容就会写到sockfd
							//2.execl之前文件重定向 execl后仍然生效
							close( STDOUT_FILENO );
							dup( sockfd );
